...
main_folder = '{}.weather_tracking'.format(main_path)
...
class Chain:
    ...
    def __init__(self, name = "default"):
        ...
        if len(self.chain_blocks) == 0:
            genesis_block = Block(
                ...
                previous_hash = "0",
                weather = {'current_condition': 'n/a', 'nearest_area': 'n/a', 'request': 'Genesis', 'weather': 'n/a'},
                comment = self.genesis_signature,
                importance = 0,
                nonce = 0
            )
    ...
    def read(self):
        for chain in self.chain_blocks:
            ...
            if chain.index != 0:
                print('[0x{:0>4}]:{:0>2}: "{}" {}'.format(h, chain.importance, chain.comment, time.ctime(chain.timestamp)))
    ...
    def block(self, comment, importance = 0):
        ...
        try:
            weather_record = " ".join(urllib.request.urlopen('http://wttr.in/{}?format=j1'.format(self.weather_channel)).read().decode('utf-8').split(' ')).strip().rstrip()
        except Exception:
            weather_record = {'current_condition': 'n/a', 'nearest_area': 'n/a', 'request': '{}'.format(self.weather_channel), 'weather': 'Unexpected error.'}
...
class Block:
    ...
    '''
    def __init__(self, index, timestamp, previous_hash, weather, comment = "", importance = 0, nonce = 0):
        self.index = index
    ...
    @property
    def weather_record(self):
        weather_record = json.loads(self.weather)
